...
if [[ -z "${NUM}" ]]; then
  echo "W: Invoked without worker number, assuming 1. Data loss may occur."
...
if [[ $? == 0 ]] ; then
  echo "E: Machine '${WORKER_NAME}' already exists."
  exit 1
fi

# Create the machine.
echo "I: Creating worker '${WORKER_NAME}'..."
...
echo "!: ${!varname}"
if [[ ! -z "${!varname}" ]]; then
  echo "I: Overriding IP"
  echo "IP='${!varname}'" >> "${TEMPFILE}"
else
...
echo "I: Configuring worker to allow storage persistance. ... "
...
if [[ -z "${URL}" ]]; then
  echo "E: Failed to determine rancher-agent status."
  exit 1
fi

echo "I: (${WORKER_NAME}) installing rancher-agent ..."
